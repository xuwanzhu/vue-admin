  ...
  methods: {
    ...
    showPwd () { //查看密码
      if (this.pwdType === 'password') {
        this.pwdType = '' //清空密码类型 才会显示数据
      } else {
        this.pwdType = 'password'
      }
    },

    ///点击登录按钮，如果没有通过我们配置的验证，即使点击登录也是无法发送请求的
    handleLogin () {
      //获取this.$refs.loginForm 里面的数据
      this.$refs.loginForm.validate(valid => { //表单验证
        //如果符合我的规则
        if (valid) {
          // debugger
          this.loading = true //显示加载图标
          //使用分发方式异步发起请求，请看store的modules下面的user.js中的Login
          //this.$store.dispatch() dispatch：异步操作，数据提交至 actions，可用于向后台提交数据
          //this.$store.dispatch(‘Login’, this.loginForm)来调取store里actions的user.js的login方法，从而要更新
