...
export default {
  ...
  data () {
    return {
      ...
      dialogFormVisible: false, //false弹窗关闭
      dialogPermissionVisible: false,
      menu: menuForm,
      permission: perForm,

      menuValidateRules: { //输入校验
  ...
  methods: {
    ...
    appendLevelOne () {
      menu.saveLevelOne(this.menu)
        .then(response => {
          this.dialogFormVisible = false //菜单弹窗
          this.$message({
            type: 'success',
            message: '添加一级菜单成功'
          })
          // 刷新页面
          this.fetchNodeList()
          this.menu = { ...menuForm }  //对象赋值copy,清空添加菜单弹窗
          this.permission = { ...perForm } //清空添加功能弹窗
        })
        .catch(response => {
          // 你们写：判断点击取消清空一下
