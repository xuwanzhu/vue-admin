  ...
  methods: {
    ...
    getById (userId) {
      userApi.getAssign(userId).then(response => {
        ...
        this.cities = response.data.allRolesList //获取显示所有角色
      })
    },
    ...
    handleCheckAllChange (val) {
      console.log("+++++++++" + val)
      this.checkedCities = val ? this.cities : []; //全部 或者空
      this.isIndeterminate = false;
    },

    //多选
    handleCheckedCitiesChange (value) { //value 为上面的label的值
      console.log("=============" + value)
      let checkedCount = value.length;

      this.checkAll = checkedCount === this.cities.length; //如果是全选则显示全选样式
    ...
    update () {
      ...
      var ids = this.checkedCities.join(",")  //选中的角色id 
      console.log(ids)
      //修改权限
