  ...
  methods: {
    ...
    init () {
      if (this.$route.params && this.$route.params.id) {
        ...
        this.fetchDataById(this.roleId) //根据角色id获取菜单
      }
    },

    fetchDataById (roleId) {
      menu.toAssign(roleId).then(response => { //根据角色获取菜单
        this.data = response.data.children //菜单数据
        var jsonList = JSON.parse(JSON.stringify(this.data))

        debugger;
        var list = []  //角色拥有的权限集合
        this.getJsonToList(list, jsonList[0]['children'])   //第0位置的children
        console.log("最终集合")
        console.log(list)
    ...
    getJsonToList (list, jsonList) {  //jsonList：第0位置的children中的7个管理
      debugger
      //遍历这个集合对象，获取key的值
      for (var i = 0; i < jsonList.length; i++) {
        if (jsonList[i]['select'] == true && jsonList[i]['level'] == 4) {  //true为选中，4也为选中
