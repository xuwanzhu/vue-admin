...
/*
Vue-Element-Admin的router总使用constantRoutes和asyncRoutes两种定义路由，
在store.modules.permission.js中对于两者的权限有默认的判断方式，然后再根目录下的permission.js中进行导航守卫逻辑；
...
/**
 *
我会从数据库中取出路由信息，然后组成下面的内容，这叫做动态拼接路由，由于不同的用户路由是不同的，
所以我们需要动态拼接，所以下面这些没有用了
 * 
 * 动态路由*/
...
export const asyncRoutes = [
  ...
  // 讲师管理
  {
    ...
    children: [
      {
        path: 'table', //在浏览器地址栏中显示的地址
        name: '讲师列表',
        component: () => import('@/views/edu/teacher/list'),
        meta: { title: '讲师列表', icon: 'table' }
      },
      {
        path: 'save', //在浏览器地址栏中显示的地址
        name: '添加讲师',
    ...
    children: [
      {
        ...
        meta: { title: '添轮播图', icon: 'table' }
      },
      {
        path: 'list',
        name: '轮播图列表',
        component: () => import('@/views/cms/banner/list'),
        meta: { title: '图轮播图列表表显示', icon: 'tree' }
      },
      {
        path: 'edit/:id',
        name: '编辑Banner',
...

  {
    ...
    children: [
      ...
      },
      {
        path: 'paylist',
        name: '支付列表',
        component: () => import('@/views/order/paylist'),
        meta: { title: '支付列表', icon: 'tree' }
      }
    ]
  },
